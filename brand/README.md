...
พื้นหลังสร้างด้วย AI (Cloudflare Workers AI รุ่น FLUX.1 schnell) แล้วนำมาเป็นเลเยอร์ล่าง
ส่วนข้อความภาษาไทย ไอคอน และเส้นแบ่งปุ่มวาดทับด้วย HTML/CSS แล้วเรนเดอร์ผ่านเบราว์เซอร์

แยกสองชั้นแบบนี้เพราะโมเดลสร้างภาพเขียนภาษาไทยไม่ได้ (สระลอย วรรณยุกต์หลุด)
และวางองค์ประกอบให้ตรงพิกัดปุ่มของ Rich Menu ไม่ได้ การซ้อนข้อความทีหลังจึงได้ทั้ง
ภาพสวยและตัวอักษรคมชัด ทั้งยังแก้ข้อความได้ทีหลังโดยไม่ต้องสร้างภาพใหม่
...
## สร้างพื้นหลังใหม่ด้วย AI

```bash
python3 ~/.claude/skills/flux-image/generate.py "<prompt ภาษาอังกฤษ>" "$PWD/ai-richmenu.jpg" 8
```

ข้อควรรู้เวลาเขียน prompt

- ปิดท้ายด้วย `no text, no letters` เสมอ เพราะโมเดลเขียนตัวอักษรไม่ได้
- พื้นหลัง Rich Menu ต้อง **สว่างสม่ำเสมอทั้งภาพ ไม่มีวัตถุเด่นตรงกลาง** เพราะจะถูกแบ่ง 6 ช่อง
- ผลลัพธ์เป็น 1024×1024 เสมอ ภาพ Rich Menu จึงถูกขยายเป็น 2500 px ตอนเรนเดอร์
  ซึ่งยอมรับได้เพราะเป็นพื้นหลังนามธรรมที่มีสีทับอยู่แล้ว
