...
export async function push(lineUserId, messages) {
  ...
  try {
    const res = await fetch(PUSH_URL, {
      ...
      body: JSON.stringify({ to: lineUserId, messages: withQuickReply(messages.slice(0, 5)) }),
    });
    if (!res.ok) {
...
export async function reply(replyToken, messages) {
  ...
  try {
    const res = await fetch(REPLY_URL, {
      ...
      body: JSON.stringify({ replyToken, messages: withQuickReply(messages.slice(0, 5)) }),
    });
    if (!res.ok) {
      const body = await res.text();
 ...
 * push() และ reply() เรียกให้อัตโนมัติแล้ว ทุกข้อความที่ระบบส่งจึงมีปุ่มลัดเสมอ
 */
