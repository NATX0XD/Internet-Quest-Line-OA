...
import { push, welcomeCard, checkinCard, quizCard, badgeCard } from './push.js';
...
export async function upsertUser(profile) {
  ...
  // ทักทายเฉพาะครั้งแรกที่ผูกบัญชี ผู้ใช้เดิมที่กลับมา login จะไม่ถูกรบกวน
  await push(profile.lineUserId, [welcomeCard(profile.displayName)]);

  return user.user_id;
}

/** หา user_id ภายในระบบจาก LINE user id คืน null เมื่อผู้ใช้ยังไม่เคยเข้าเว็บ */
export async function userIdByLineId(lineUserId) {
  const users = await readAll(SHEETS.USERS);
  const u = findOne(users, 'line_user_id', lineUserId);
...
export async function checkin(userId) {
  ...
  const dashboard = dashboardFrom(db, userId);

  await notify(user.line_user_id, [
    checkinCard({ points, bonus, streak, balance: dashboard.points, level: dashboard.level }),
    ...(rewards.newBadges.length ? [badgeCard(rewards.newBadges)] : []),
  ]);

  return { points, bonus, streak, balance, ...rewards, dashboard };
}

/** ส่งแจ้งเตือนเข้าแชท โดยไม่ให้ความล้มเหลวกระทบผลลัพธ์ของกิจกรรมหลัก */
async function notify(lineUserId, messages) {
  try {
    await push(lineUserId, messages);
  } catch (err) {
    console.error('[notify] แจ้งเตือนไม่สำเร็จ:', err.message);
  }
}
...
export async function submitQuiz(userId, categoryId, answers, durationSeconds) {
  ...
  const user = findOne(db[SHEETS.USERS], 'user_id', userId);
  await notify(user?.line_user_id, [
    quizCard({ categoryName: cat.name, score, total, percent, passed, points, balance: dashboard.points }),
    ...(rewards.newBadges.length ? [badgeCard(rewards.newBadges)] : []),
  ]);
